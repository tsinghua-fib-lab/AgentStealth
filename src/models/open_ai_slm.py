...
class OpenAISLM(BaseModel):
    def __init__(self, config: ModelConfig):
        ...
        if "max_tokens" not in self.config.args.keys():
            self.config.args["max_tokens"] =1024 
        if "url" not in self.config.args.keys():
            self.config.args["url"] ="http://0.0.0.0:8001/v1"   

    def _predict_call(self, input: List[Dict[str, str]]) -> str:
        client = OpenAI(
        base_url=self.config.args["url"], 
